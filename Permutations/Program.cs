...
namespace StringPattern
{
    class Program
    {
        ...
        {
            Func<string, string, string> pf = FindPattern;
            pf += CalcPattern;

            Console.WriteLine(pf("use", "question"));
            Console.WriteLine(pf("use", "sugestion"));
            Console.WriteLine(pf("use", "usgues"));
        }

        static string FindPattern(string pattern, string text)//O(n4)
        {
            int match = 0;
        ...
        static string CalcPattern(string pattern, string text) //O(2n2)
        {
            int p = 0, m = 0;
